/**************************************************************************************************
*
* \file CRTP_Concept.cpp
* \brief C++ Training - Example for the CRTP Design Pattern based on C++20 concepts
*
...
class DenseVectorTag
{};

template< typename T >
concept DenseVector =
   std::derived_from<T,DenseVectorTag> and
   requires( T const t, size_t index ) {
      { t.size() } -> std::integral;
      t[index];
      // ... checks for the complete required interface
   };

template< DenseVector VectorType >
std::ostream& operator<<( std::ostream& os, VectorType const& vector )
{
...
template< DenseVector VectorType >
constexpr decltype(auto) l2norm( VectorType const& vector )
{
...
template< DenseVector VT1, DenseVector VT2 >
VecVecAddExpr<VT1,VT2> operator+( VT1 const& lhs, VT2 const& rhs )
{
   if( lhs.size() != rhs.size() ) {
      throw std::invalid_argument( "Vector size does not match" );
   }

   return VecVecAddExpr<VT1,VT2>( lhs, rhs );
}
