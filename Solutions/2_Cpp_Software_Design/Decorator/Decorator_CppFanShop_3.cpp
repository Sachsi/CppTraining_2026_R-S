/**************************************************************************************************
*
* \file Decorator_CppFanShop_3.cpp
...
class Money
{
 public:
   ...
   constexpr double value() const { return static_cast<double>(value_)/100.0; }
...
template< typename T >
struct IsItem : public decltype( isItem( std::declval<T*>() ) )
{};

template< typename T >
struct IsItem<T&> : public std::false_type
{};
...
class Item
{
 public:
   template< typename T >
   using enable_if_not_is_item = std::enable_if_t< not IsItem_v<T>, bool >;
   ...
   Item& operator=( Item const& item ) { pimpl_ = item.pimpl_->clone(); return *this; }
...
{
 public:
   explicit DecoratedItem( Item item )
      : item_{ std::move(item) }
   {}
...
{
 public:
   Discount( double discount, Item item )
      : DecoratedItem( std::move(item) )
      , factor_( 1.0 - discount )
   {
      if( not std::isfinite(discount) or discount < 0.0 or discount > 1.0 ) {
...
{
 public:
   Tax( double tax, Item item )
      : DecoratedItem( std::move(item) )
      , factor_( 1.0 + tax )
   {
      if( not std::isfinite(tax) or tax < 0.0 ) {
...
{
 public:
   QuantityDiscount( size_t count, Item item )
      : DecoratedItem{ std::move(item) }
      , count_{ count }
   {}
...
int main()
{
   Item item1( Tax( 0.19, Discount( 0.2, ConferenceTicket{ "CppCon", Money{1800.0F} } ) ) );
   Item item2( Tax( 0.07, QuantityDiscount( 3, CppBook{ "Effective C++", Money{19.0F} } ) ) );
   Item item3( Tax( 0.19, CppStandard{ "ISO/IEC 14882:2020", Money{200.0F} } ) );
   Item item4( Tax( 0.19, Coupon( Money{5.0F}, CppShirt( Color::red, Money{15.0F} ) ) ) );

   Item item5( ShoppingCart( item1, item3, item4 ) );
