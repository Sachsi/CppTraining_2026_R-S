...
class DynamicVector : public DenseVector< DynamicVector<Type> >
{
 public:
...
{
   if( dst.size() != lhs.size() or lhs.size() != rhs.size() )
...
template< typename VT >
struct VecSqrtExpr : public DenseVector< VecSqrtExpr<VT> >
{
 public:
...
template< typename VT1, typename VT2, typename BinaryOperation >
struct VecVecExpr : public DenseVector< VecVecExpr<VT1,VT2,BinaryOperation> >
{
 public:
   using value_type = decltype( std::declval<BinaryOperation>()( std::declval<VT1>()[0U], std::declval<VT2>()[0U] ) );

   VecVecExpr( VT1 const& lhs, VT2 const& rhs )
      : lhs_( lhs )
      , rhs_( rhs )
      , op_{}
   {
...
VecVecExpr<VT1,VT2,std::plus<>> operator+( DenseVector<VT1> const& lhs, DenseVector<VT2> const& rhs )
{
   if( lhs.size() != rhs.size() )
      throw std::invalid_argument( "Vector size does not match" );

   return VecVecExpr<VT1,VT2,std::plus<>>( lhs.derived(), rhs.derived() );
}

template< typename VT1, typename VT2 >
VecVecExpr<VT1,VT2,std::minus<>> operator-( DenseVector<VT1> const& lhs, DenseVector<VT2> const& rhs )
{
   if( lhs.size() != rhs.size() )
      throw std::invalid_argument( "Vector size does not match" );

   return VecVecExpr<VT1,VT2,std::minus<>>( lhs.derived(), rhs.derived() );
}
