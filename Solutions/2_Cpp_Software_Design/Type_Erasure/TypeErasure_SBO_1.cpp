...
class Shape
{
 public:
   ...
   Shape( ShapeT shape, DrawStrategy drawer, SerializationStrategy serializer )
   {
      using M = ShapeModel<ShapeT,DrawStrategy,SerializationStrategy>;
      static_assert( sizeof(M) <= buffersize, "Given type is too large" );
      static_assert( alignof(M) <= alignment, "Given type is overaligned" );
      M* model = ::new (pimpl()) M( std::move(shape), std::move(drawer), std::move(serializer) );  // Placement new or C++20 'std::construct_at()'

      // While it is defined behavior to reinterpret_cast the byte buffer to a ShapeModel*,
      // it is not guaranteed that the ShapeConcept base class is at the beginning of the
      // ShapeModel class. Therefore the reinterpret_cast to a ShapeConcept* is undefined
      // behavior. However, all major compilers handle base classes this way. Therefor it
      // may be sufficient to explicitly check that the conversion works. Alternatively,
      // the pointer returned via the placement new can be stored as a data member (see
      // <TypeErasure_SBO_2.cpp>.
      assert( static_cast<ShapeConcept*>(model) == pimpl() );
   }

   ~Shape() { pimpl()->~ShapeConcept(); }  // Or C++17 'std::destroy_at()'
 ...
 private:
   ...
   */
   ...
   {
      explicit ShapeModel( ShapeT shape, DrawStrategy drawer, SerializationStrategy serializer )
      ...
      void clone( ShapeConcept* memory ) const final { ::new (memory) ShapeModel(*this); }  // Or C++20 'std::construct_at()'
   ...
   {
      return std::launder( reinterpret_cast<ShapeConcept*>( buffer.data() ) );
   }

   const ShapeConcept* pimpl() const noexcept
   {
      return std::launder( reinterpret_cast<const ShapeConcept*>( buffer.data() ) );
   }
