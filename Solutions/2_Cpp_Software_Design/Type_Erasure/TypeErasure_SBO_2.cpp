...
class Shape
{
 public:
   ...
   Shape( ShapeT shape, DrawStrategy drawer, SerializationStrategy serializer )
   {
      ...
      pimpl_ = ::new (buffer.data()) M( std::move(shape), std::move(drawer), std::move(serializer) );  // Placement new or C++20 'std::construct_at()'
   }

   ~Shape() { pimpl_->~ShapeConcept(); }  // Or C++17 'std::destroy_at()'
 ...
 private:
   ...
   */
   ...
   struct ShapeConcept
   {
      ...
      virtual ShapeConcept* clone( void* memory ) const = 0;
   };
   ...
   struct ShapeModel final : public ShapeConcept
   {
      ...
      ShapeConcept* clone( void* memory ) const final { return ::new (memory) ShapeModel(*this); }  // Or C++20 'std::construct_at()'
   ...
   ShapeConcept* pimpl_;  // Stores a pointer to the ShapeModel created in the constructor.
