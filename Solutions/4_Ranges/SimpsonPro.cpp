...
void print( Persons const& table )
{
...
void random_order( Persons& table )
{
...
void find_youngest( Persons const& table )
{
...
void order_by_lastname( Persons& table )
{
...
void highlight_lastname( Persons& table )
{
   std::string lastname{};
...
void children_first( Persons& table )
{
   ...
   std::partition( std::begin(table), std::end(table),
      []( Person const& person )
      {
         return person.age > 5 and person.age < 18;
      } );
...
void compute_total_lastname_length( Persons const& table )
{
   // TODO: Compute the total length of all last names ('t')

   auto const total_length =
      std::transform_reduce( std::begin(table), std::end(table)
...
void same_age( Persons const& table )
{
...
void find_simpsons( Persons& table )
{
...
void maximum_age_difference( Persons const& table )
{
...
void print_children( Persons const& table )
{
...
int main()
{
   ...
   while( std::cout << "Enter command: " and std::cin >> command )
   {
      switch( command )
      {
      case 'r':
