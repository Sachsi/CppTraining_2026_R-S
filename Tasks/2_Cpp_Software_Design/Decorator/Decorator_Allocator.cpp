...
constexpr auto next_multiple( T1 value, T2 factor ) noexcept
{
...
class AlignedAllocator
{
   ...
   // Note 1: 'std::aligned_alloc' is not available on MSVC (see the notes on
   //         https://en.cppreference.com/w/cpp/memory/c/aligned_alloc). You will have
   //         to use '_aligned_malloc()' and '_aligned_free()'.
   // Note 2: POSIX memalign, which might be used within 'std::aligned_alloc', requires
   //         the alignment to be a power of two (see the 'is_power_of_two()' helper
   //         function) and the requested number of bytes to be a multiple of the
   //         alignment (see the 'next_multiple()' helper function).
};
