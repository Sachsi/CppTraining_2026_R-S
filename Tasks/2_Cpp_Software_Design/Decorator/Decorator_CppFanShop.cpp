...
// Step 1: Implement a 'Discount' decorator, which represents an item-specific discount
//         between 0 and 100%.
...
// Step 1: Implement a 'Coupon' decorator, which represents a fixed amount of money that
//         can be used to reduce a price.
...
// Step 1: Implement a 'Tax' decorator, which represents an item-specific tax rate.
...
{
   // Step 2: Introduce a quantity discount for an item that is selected multiple times
...
class ShoppingCart
{
   // Step 2: Introduce a discount for different items put into a shopping basket
   //         (2 items: 10%, 3 items: 15%, 4 items: 20%, more than four items: 25%).
};
...
int main()
{
   std::unique_ptr<Item> item1(
      std::make_unique<ConferenceTicket>( "CppCon", Money{1800.0F} ) );
