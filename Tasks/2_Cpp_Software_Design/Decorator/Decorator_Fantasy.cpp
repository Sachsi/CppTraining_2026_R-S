...
int main()
{
   ...
   std::unique_ptr<Weapon> knightWeapon = std::make_unique<Sword>();

   std::unique_ptr<Armor> knightArmor = std::make_unique<Shield>();

   std::unique_ptr<Gear> knightGear = std::make_unique<LeatherBoots>();
   ...
   std::unique_ptr<Gear> dragonGear = std::make_unique<DragonSpeed>();
